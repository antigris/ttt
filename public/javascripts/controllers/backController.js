...
angular
  .module('ttt')
  .controller('backController', backController);

function backController ($sessionStorage, $document) {
 var w = window.innerWidth;
        ...
        var blower= [];
        for(let a=0;a<500;a++)blower.push({'px': w/2, 'py': h/2, 'vx': 5 -10*Math.random(),'vy': 5 -10*Math.random()});
        ...
        function draw() {
            ...
            if(dir==-1) {
                ...
                oy -= state.velocity;
                let f=-1;
                for(let n=-h; n<h*2; n+=400*f){
                     
                    for(let m=-w/5; m<w; m+=50*f){
                        f=f*f;
                        cx = ox + m*f + m*Math.random();
                        cy = oy - n*f + n*Math.random();
                        ctx.beginPath();
                        for(let s=1;s<4;s++){
                            ctx.arc(cx,cy+4*s ,2*s, 0, 2*Math.PI, false);
                        }
                        ctx.closePath();
                        ctx.fill();
                    }
